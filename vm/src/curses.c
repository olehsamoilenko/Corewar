...
void	print_memory(t_war *war)
{
	...
	while (player < 4)
	{
		...
		{
			mvwprintw(war->win_info, line, 4, "Last live : %21d", war->champs[player]->last_live);
			// itoa = ft_itoa(war->champs[player]->last_live);
			// mvwaddstr(war->win_info, line, 37 - ft_strlen(itoa), itoa);
			// ft_strdel(&itoa);
			line++;
			mvwprintw(war->win_info, line, 4, "Lives in current period : %7d", war->champs[player]->lives_cur_period);
